 ...
 * Your own data store doesn't need to use ER_Data_Store_WP -- you can write
...
/**
 * ER_Data_Store_WP class.
 */
class ER_Data_Store_WP {
	 ...
	 *
	 * @param ER_Data|integer $object ER_Data object or object ID.
	 * @param string          $taxonomy Taxonomy name e.g. resource_cat.
	 *
	 ...
	 *
	 * @param ER_Data $object ER_Data object.
	 *
	 * @return array
	 */
	...
	public function filter_raw_meta_data( &$object, $raw_meta_data ) {
		...
		return apply_filters( "easyreservations_data_store_wp_{$this->meta_type}_read_meta", $meta_data, $object, $this );
	}
	...
	/**
	 * Get valid WP_Query args from a ER_Object_Query's query variables.
	 *
	 * @param array $query_vars query vars from a ER_Object_Query.
	 *
	 * @return array
	 */
	protected function get_wp_query_args( $query_vars ) {
		...
		return apply_filters( 'easyreservations_get_wp_query_args', $wp_query_args, $query_vars );
	}
	 ...
	 * Also accepts a ER_DateTime object.
	 *
	...
	public function parse_date_for_wp_query( $query_var, $key, $wp_query_args = array() ) {
		...
		// YYYY-MM-DD queries have 'day' precision. Timestamp/ER_DateTime queries have 'second' precision.
		$precision = 'second';
		...
		try {
			// Specific time query with a ER_DateTime.
			if ( is_a( $query_var, 'ER_DateTime' ) ) {
				$dates[] = $query_var;
			} elseif ( is_numeric( $query_var ) ) { // Specific time query with a timestamp.
				$dates[] = new ER_DateTime( "@{$query_var}", new DateTimeZone( 'UTC' ) );
			} elseif ( preg_match( $query_parse_regex, $query_var, $sections ) ) { // Query with operators and possible range of dates.
				if ( ! empty( $sections[1] ) ) {
					$dates[] = is_numeric( $sections[1] ) ? new ER_DateTime( "@{$sections[1]}", new DateTimeZone( 'UTC' ) ) : er_string_to_datetime( $sections[1] );
				}

				$operator = in_array( $sections[2], $valid_operators, true ) ? $sections[2] : '';
				$dates[]  = is_numeric( $sections[3] ) ? new ER_DateTime( "@{$sections[3]}", new DateTimeZone( 'UTC' ) ) : er_string_to_datetime( $sections[3] );

				if ( ! is_numeric( $sections[1] ) && ! is_numeric( $sections[3] ) ) {
					$precision = 'day';
				}
			} else { // Specific time query with a string.
				$dates[]   = er_string_to_datetime( $query_var );
				$precision = 'day';
			}
		} catch ( Exception $e ) {
	...
	protected function get_valid_search_terms( $terms ) {
		...
		foreach ( $terms as $term ) {
			...
			if ( in_array( er_strtolower( $term ), $stopwords, true ) ) {
				continue;
			}
	...
	protected function string_to_timestamp( $time_string ) {
		return '0000-00-00 00:00:00' !== $time_string ? er_string_to_timestamp( $time_string ) : null;
	}
}
