...
class ER_HTTPS {
	...
	public static function init() {
		if ( 'yes' === get_option( 'reservations_force_ssl_checkout' ) && ! is_admin() ) {
			...
			if ( 'yes' == get_option( 'reservations_unforce_ssl_checkout' ) ) {
	...
	public static function force_https_url( $content ) {
		if ( is_ssl() ) {
			if ( is_array( $content ) ) {
				$content = array_map( 'ER_HTTPS::force_https_url', $content );
			} else {
	...
	public static function force_https_page_link( $link, $page_id ) {
		if ( in_array( $page_id, array( get_option( 'reservations_checkout_page_id' ), get_option( 'reservations_myaccount_page_id' ) ) ) ) {
	...
	public static function force_https_template_redirect() {
		if ( ! is_ssl() && ( is_easyreservations_checkout() || is_easyreservations_account_page() || apply_filters( 'easyreservations_force_ssl_checkout', false ) ) ) {
