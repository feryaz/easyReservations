...
/**
 * ER_Tracks_Event class.
 */
class ER_Tracks_Event {
	...
	public function record() {
		if ( wp_doing_ajax() || defined( 'REST_REQUEST' ) ) {
			return ER_Tracks_Client::record_event( $this );
		}

		return ER_Tracks_Footer_Pixel::record_event( $this );
	}
	...
	public static function validate_and_sanitize( $event ) {
		...
		$validated = array(
			'browser_type' => ER_Tracks_Client::BROWSER_TYPE,
		);
		...
		if ( ! isset( $_event->_ts ) ) {
			$_event->_ts = ER_Tracks_Client::build_timestamp();
		}
	...
	public function build_pixel_url() {
		...
		return esc_url_raw( ER_Tracks_Client::PIXEL . '?' . http_build_query( $validated ) );
	}
