<?php
/**
 * easyReservations Admin Setup Wizard Tracking
 *
...
/**
 * This class adds actions to track usage of the easyReservations Onboarding Wizard.
 */
class ER_Admin_Setup_Wizard_Tracking {
	/**
	...
	public function init() {
		...
		add_filter( 'easyreservations_setup_wizard_steps', array( $this, 'set_obw_steps' ) );
		add_action( 'shutdown', array( $this, 'track_skip_step' ), 1 );
		add_action( 'add_option_easyreservations_allow_tracking', array( $this, 'track_start' ), 10, 2 );
		...
		add_action( 'easyreservations_setup_footer', array( $this, 'add_footer_scripts' ) );
	}
	...
	/**
	 * Add footer scripts to OBW via easyreservations_setup_footer
	 */
	public function add_footer_scripts() {
		wp_print_scripts();
		ER_Site_Tracking::add_tracking_function();
		er_print_js();
	}
	...
	public function track_start( $option, $value ) {
		if ( 'yes' !== $value || empty( $_GET['page'] ) || 'er-setup' !== $_GET['page'] ) { // phpcs:ignore WordPress.Security.NonceVerification.Recommended
			return;
		}

		ER_Tracks::record_event( 'obw_start' );
	}
	...
	public function track_ready_next_steps() {
		...
		er_enqueue_js(
			...
			jQuery( '.er-setup-content a' ).on( 'click', function trackNextScreen( e ) {
				...
				window.erTracks.recordEvent( 'obw_ready_next_step', properties );
			} );
		"
		);
	}
	...
	public function add_step_save_events() {
		...
		update_option( 'reservations_obw_last_completed_step', $this->get_current_step() );
	...
	public function track_store_setup() {
		...
		ER_Tracks::record_event( 'obw_store_setup', $properties );
	}
	...
	public function track_payments() {
		...
		$er_admin_setup_wizard = new ER_Admin_Setup_Wizard();
		$gateways              = array_merge( $er_admin_setup_wizard->get_wizard_in_cart_payment_gateways(), $er_admin_setup_wizard->get_wizard_manual_payment_gateways() );

		foreach ( $gateways as $gateway_id => $gateway ) {
			if ( ! empty( $_POST[ 'er-wizard-service-' . $gateway_id . '-enabled' ] ) ) { // WPCS: CSRF ok, input var ok.
		...
		if (
			! empty( $_POST['er-wizard-service-stripe-enabled'] ) && // WPCS: CSRF ok, input var ok.
		...
		if (
			! empty( $_POST['er-wizard-service-ppec_paypal-enabled'] ) && // WPCS: CSRF ok, input var ok.
		...
		ER_Tracks::record_event( 'obw_payments', $properties );
	}
	...
	public function track_shipping() {
		...
		$properties = array(
			...
			'setup_ers_labels'  => isset( $_POST['setup_easyreservations_services'] ) && 'yes' === $_POST['setup_easyreservations_services'],
			'setup_shipstation' => isset( $_POST['setup_shipstation'] ) && 'yes' === $_POST['setup_shipstation'],
		);
		// phpcs:enable

		ER_Tracks::record_event( 'obw_shipping', $properties );
	}
	...
	public function track_recommended() {
		...
		$properties = array(
			...
			'setup_er_admin'      => isset( $_POST['setup_er_admin'] ) && 'yes' === $_POST['setup_er_admin'],
		);
		// phpcs:enable

		ER_Tracks::record_event( 'obw_recommended', $properties );
	}
	...
	public function track_jetpack_activate() {
		ER_Tracks::record_event( 'obw_activate' );
	}
	...
	public function track_next_steps() {
		ER_Tracks::record_event( 'obw_ready_view' );
	}
	...
	public function track_skip_step() {
		$previous_step = get_option( 'reservations_obw_last_completed_step' );
		...
		if ( $current_step_index > $previous_step_index + 1 ) {
			...
			ER_Tracks::record_event( 'obw_skip_step', $properties );
		}
	}
