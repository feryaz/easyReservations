<?php
/**
 * Class ER_Email_Failed_Order file.
 *
...
if ( ! class_exists( 'ER_Email_Failed_Order', false ) ) :
	 ...
	 *
	 * @class       ER_Email_Failed_Order
	 * @package     easyReservations/Classes/Emails
	 * @extends     ER_Email
	 */
	class ER_Email_Failed_Order extends ER_Email {
		...
		public function __construct() {
			...
			$this->title          = __( 'Failed order', 'easyReservations' );
			$this->description    = __( 'Failed order emails are sent to chosen recipient(s) when orders have been marked failed (if they were previously pending or on-hold).', 'easyReservations' );
			...
			add_action( 'easyreservations_order_status_pending_to_failed_notification', array( $this, 'trigger' ), 10, 2 );
			add_action( 'easyreservations_order_status_on-hold_to_failed_notification', array( $this, 'trigger' ), 10, 2 );
		...
		public function get_default_subject() {
			return __( '[{site_title}]: Order #{order_number} has failed', 'easyReservations' );
		}
		...
		public function get_default_heading() {
			return __( 'Order Failed: #{order_number}', 'easyReservations' );
		}
...
return new ER_Email_Failed_Order();
