...
class ER_Admin_Permalink_Settings {
	...
	public function settings() {
		...
		<table class="form-table er-permalink-structure">
			<tbody>
				<tr>
					<th><label><input name="resource_permalink" type="radio" value="<?php echo esc_attr( $structures[0] ); ?>" class="ertog" <?php checked( $structures[0], $this->permalinks['resource_base'] ); ?> /> <?php esc_html_e( 'Default', 'easyReservations' ); ?></label></th>
					<td><code class="default-example"><?php echo esc_html( home_url() ); ?>/?resource=sample-resource</code> <code class="non-default-example"><?php echo esc_html( home_url() ); ?>/<?php echo esc_html( $resource_base ); ?>/sample-resource/</code></td>
				</tr>
				<?php if ( $shop_page_id ) : ?>
					<tr>
						<th><label><input name="resource_permalink" type="radio" value="<?php echo esc_attr( $structures[1] ); ?>" class="ertog" <?php checked( $structures[1], $this->permalinks['resource_base'] ); ?> /> <?php esc_html_e( 'Shop base', 'easyReservations' ); ?></label></th>
						<td><code><?php echo esc_html( home_url() ); ?>/<?php echo esc_html( $base_slug ); ?>/sample-resource/</code></td>
					</tr>
				<?php endif; ?>
				<tr>
					<th><label><input name="resource_permalink" id="easyreservations_custom_selection" type="radio" value="custom" class="tog" <?php checked( in_array( $this->permalinks['resource_base'], $structures, true ), false ); ?> />
						<?php esc_html_e( 'Custom base', 'easyReservations' ); ?></label></th>
					<td>
						<input name="resource_permalink_structure" id="easyreservations_permalink_structure" type="text" value="<?php echo esc_attr( $this->permalinks['resource_base'] ? trailingslashit( $this->permalinks['resource_base'] ) : '' ); ?>" class="regular-text code"> <span class="description"><?php esc_html_e( 'Enter a custom base to use. A base must be set or WordPress will use default instead.', 'easyReservations' ); ?></span>
					</td>
				</tr>
			</tbody>
		</table>
