...
class ER_Validation {
	...
	public static function is_postcode( $postcode, $country ) {
		...
		switch ( $country ) {
			...
			case 'HU':
				...
				break;
			case 'IE':
				$valid = (bool) preg_match( '/([AC-FHKNPRTV-Y]\d{2}|D6W)[0-9AC-FHKNPRTV-Y]{4}/', er_normalize_postcode( $postcode ) );
				break;
			case 'IN':
		...
		return apply_filters( 'easyreservations_validate_postcode', $valid, $postcode, $country );
	}
	...
	public static function format_postcode( $postcode, $country ) {
		return er_format_postcode( $postcode, $country );
	}
	...
	public static function format_phone( $tel ) {
		return er_format_phone_number( $tel );
	}
}
