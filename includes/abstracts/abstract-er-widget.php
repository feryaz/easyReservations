 ...
 * @extends  WP_Widget
 */
abstract class ER_Widget extends WP_Widget {
	...
	public function update( $new_instance, $old_instance ) {
		...
		foreach ( $this->settings as $key => $setting ) {
			 ...
			 */
			$instance[ $key ] = apply_filters( 'easyreservations_widget_settings_sanitize_option', $instance[ $key ], $new_instance, $key, $setting );
		}
	...
	public function form( $instance ) {
		...
		foreach ( $this->settings as $key => $setting ) {
			...
			switch ( $setting['type'] ) {
				...
				default:
					do_action( 'easyreservations_widget_field_' . $setting['type'], $key, $value, $setting, $instance );
					break;
			}
		}
	}
	...
	/**
	 * Get current page URL with various filtering props supported by ER.
	 *
	 * @return string
	 */
	protected function get_current_page_url() {
		...
		if ( isset( $_GET['min_price'] ) ) {
			$link = add_query_arg( 'min_price', er_clean( wp_unslash( $_GET['min_price'] ) ), $link );
		}

		if ( isset( $_GET['max_price'] ) ) {
			$link = add_query_arg( 'max_price', er_clean( wp_unslash( $_GET['max_price'] ) ), $link );
		}

		// Order by.
		if ( isset( $_GET['orderby'] ) ) {
			$link = add_query_arg( 'orderby', er_clean( wp_unslash( $_GET['orderby'] ) ), $link );
		}
		...
		if ( isset( $_GET['post_type'] ) ) {
			$link = add_query_arg( 'post_type', er_clean( wp_unslash( $_GET['post_type'] ) ), $link );

			// Prevent post type and page id when pretty permalinks are disabled.
			if ( is_easyreservations_shop() ) {
				$link = remove_query_arg( 'page_id', $link );
			}
		}

		return apply_filters( 'easyreservations_widget_get_current_page_url', $link, $this );
	}
	...
	protected function get_widget_id_for_cache( $widget_id, $scheme = '' ) {
		...
		return apply_filters( 'easyreservations_cached_widget_id', $widget_id_for_cache );
	}
}
